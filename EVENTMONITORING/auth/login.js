...
async function loginUser(username, password) {
    ...
    let adminQuery = supabaseClient.from('admins').select('*').eq('password', password);
    adminQuery = isEmployeeId
        ? adminQuery.eq('employee_id', username)
        : adminQuery.eq('email', username);

    let profQuery = supabaseClient.from('professors').select('*').eq('password', password);
    profQuery = isEmployeeId
        ? profQuery.eq('employee_id', username)
        : profQuery.eq('email', username);

    let studentQuery = supabaseClient.from('students').select('*').eq('password', password);
    if (isStudentId) {
    ...
    const userData  = adminData || profData || studentData;
    const tableName = adminData   ? 'admins'
                    : profData    ? 'professors'
                    : studentData ? 'students'
                    : '';
    const userRole  = adminData ? (adminData.admin_level || 'admin')
                    : profData  ? (profData.role         || 'faculty')
                    :               'student';
    ...
    if (userData.department_id) {
        try {
            await fetchDepartmentInfoAsync(userData.department_id);
            console.log('[loginUser] ✓ Department info loaded for', tableName);
        } catch (err) {
            console.warn('[Dept] Department fetch failed:', err);
        }
    }

    if (tableName === 'admins' || tableName === 'professors') {
        writeLoginAudit(userObj, tableName).catch(err => {
